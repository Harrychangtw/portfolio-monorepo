<!--
  GitHub Copilot Instructions for Portfolio Monorepo (harrychang.me, emilychang-me)
  Last updated: 2025-11-17
-->

# AI Agent Coding Guide: Portfolio Monorepo

## Architecture Overview

- **Turbo monorepo**: Multiple Next.js 15 apps (e.g., `harrychang-me`, `emilychang-me`) managed via Turborepo.
- **Dual-domain, single codebase**: `harrychang.me` (main) and `lab.harrychang.me` (lab) are served from one Next.js app using middleware-based routing.
- **File-based CMS**: Markdown content in `content/` with YAML frontmatter, auto-localized by filename suffix (`_zh-tw.md`).
- **Custom i18n**: Client-only React Context (`contexts/LanguageContext.tsx`), dynamic JSON loading from `/public/locales/`, no server-side i18n.
- **Image optimization**: Build-time WebP/thumbnail generation (`scripts/optimize-images.js`), runtime path transforms in `lib/markdown.ts`.
...
1. Add `content/projects/my-project.md` (see frontmatter schema in `README.md`)
