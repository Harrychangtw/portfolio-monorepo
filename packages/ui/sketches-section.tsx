...
export default function SketchesSection({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      setDisplayedSketches((prev) => {
        ...
        const availableIndexes = [...Array(9).keys()].filter(
          (i) => i !== currentHoveredIndex,
        );
  ...
  const getSketchForSlot = (slotIndex: number) =>
    displayedSketches[slotIndex] || null;
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-[var(--column-spacing)]">
              {[0, 1, 2].map((columnIndex) => (
                <div
                  key={columnIndex}
                  className="space-y-[var(--column-spacing)]"
                >
                  {[columnIndex, columnIndex + 3, columnIndex + 6].map(
                    (slotIndex) => {
