...
export default function BlogCard({
    ...
    const isEffectivelyLocked = locked ? (!isClient || timeLeft === null || timeLeft > 0) : false
    ...
    const formatCountdown = (ms: number) => {
        ...
        return (
            <>
                {d > 0 && <>{String(d).padStart(2, "0")}<span className="text-foreground/30 mx-[2px]">:</span></>}
                {String(h).padStart(2, "0")}<span className="text-foreground/30 mx-[2px]">:</span>
                {String(m).padStart(2, "0")}<span className="text-foreground/30 mx-[2px]">:</span>
            ...
            <Wrapper {...wrapperProps}>
                ...
                <div className="flex-grow grid grid-cols-[1fr_auto] gap-6 mb-2">
                    <div className="flex flex-col justify-between min-w-0">
                        <h3 className={`font-heading text-base md:text-lg font-medium leading-snug tracking-wide line-clamp-4 mb-6 transition-colors duration-700 ${isEffectivelyLocked ? "text-primary/50" : "text-primary"}`}>
                            {title}
                        </h3>
                ...
                <motion.div
                    ...
                    whileHover={
                        isEffectivelyLocked
                            ? undefined
                            : { scale: 0.98, transition: { duration: 0.2, ease: [0.4, 0, 0.6, 1] } }
                    }
                >
                    <ImageContainer
                        src={imageUrl}
                        ...
                        imgClassName={isEffectivelyLocked ? "grayscale transition-all duration-700" : "transition-all duration-700"}
                    />
                            ...
                            >
                                <div className="relative flex flex-col items-center justify-center w-[85%] h-[80%] max-w-[260px] max-h-[160px]">
                                    ...
                                    <div className="font-mono text-[13px] tracking-[0.2em] text-foreground tabular-nums mt-1 flex items-center">
                                        {timeLeft === null ? (
                                            <span className="animate-pulse opacity-60 tracking-[0.3em]">COMPUTING</span>
                                        ) : (
