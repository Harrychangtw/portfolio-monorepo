...
category: "學術研究"
subcategory: "LLM 安全"
description: "FORTRESS 是我發表於 TMLR 的無需訓練 LLM 安全系統。透過單一 IT 模型整合了向量檢索與動態 perplexity 分析，打造可擴展的高效能防禦機制，速度比既有分類器快上五倍以上。"
...
featured: true
locked: false
tooltip: "發表於 TMLR 的第一作者研究：一個無需訓練的高速 LLM 安全系統。"
...
1.  **無需訓練且可擴展：** LLM 安全是一個不斷演變的領域。現有方法難以應對新型攻擊技術，而要彌補這些防禦缺口需要進行昂貴的訓練。與此形成鮮明對比的是，擴展 FORTRESS 就像將新資料加入到其向量資料庫一樣簡單，無需任何其他修改。這種方法不但能有效防禦新攻擊，還能確保在既有 benchmark 上的效能惡化降至最低。

2.  **高運算效率：** 輕量級語言模型的興起推動了外部安全防護機制的發展，這些機制旨在將額外開銷降至最低。FORTRESS 使用小至 0.6B 參數的模型，就能達到遠超其規模的模型的結果。
...
![FORTRESS 系統架構圖，說明了資料策展、資料擴展與偵測流水線階段。使用者輸入由 LLM 引擎處理，以生成用於主要向量搜尋的向量嵌入與用於次要 perplexity 分析的 log-probabilities。](images/optimized/projects/2025_07_04_fortress_system/fortress_system_diagram.webp)
...
最引人注目的發現是**動態、依類別劃分的 perplexity 臨界值**的重要性。移除此功能並改用單一的全域臨界值，導致平均 F1 score 出現了**災難性的 14.7 個百分點**下降。這證明了情境感知分析是 FORTRESS 高精確度的基礎。此外，結果也驗證了我們使用整合式 instruction-tuned LLM 的選擇，其表現優於使用傳統、僅限檢索的 embedding 模型的架構。

### 結論與未來研究方向

FORTRESS 為 LLM 安全建立了一個同時具備穩健、高效與適應性的新典範。透過在一個無需訓練的框架中整合向量檢索與動態 perplexity 分析，它在保持可擴展性與運算效率的同時，達到了前沿的效能。
...
投稿過程同樣像在黑暗中前進。原本打算投 AAAI 2026，後來考量時程與篇幅轉投 TMLR。第一次投期刊的經驗其實不錯：AE 推薦後不久就收到兩份正面回覆；第三位審稿人的建議也讓我補上關鍵實驗。但後續的等待超乎預期 —— 多次延期，還一度以為要到十一月底才有結果。當時我正忙於大學申請，焦慮至極；所幸在 10 月 11 日收到錄取通知，從一個模糊的念頭走到被接受，這段路雖然曲折，卻也使成就感無比難忘。
