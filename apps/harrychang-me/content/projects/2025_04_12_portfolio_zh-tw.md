...
- [**專案列表**](/#projects)：展示所有專案，每個專案都有獨立的頁面，包含詳細的描述、圖片與影片。
- [**攝影畫廊**](/#gallery)：一系列影像的集合，每張圖片都有專屬的頁面進行詳細展示，並透過客製化的畫框系統提升視覺呈現效果。
- [**部落格**](/#blog)：涵蓋各種主題的文章區域，從評論到各式隨筆。
- [**理念**](/manifesto)：一個專屬頁面，用來表達驅動我的核心原則，讓讀者能更專注地閱讀。
- [**論文閱讀**](/paper-reading)：動態更新的學術論文閱讀筆記列表。
- [**工具與設定**](/uses)：列出我日常使用的工具與軟體。
