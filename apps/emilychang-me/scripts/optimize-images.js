 ...
 * It processes images in the public/images/projects, public/images/gallery, and public/images/sketches folders.
 * 
 * Usage:
...
const config = {
  ...
  directories: {
    ...
    sketchesSource: path.join(process.cwd(), 'public', 'images', 'sketches'),
...
async function processSketchesImages() {
  console.log('Processing sketches images...');
  
  if (!fs.existsSync(config.directories.sketchesSource)) {
    console.log('Sketches directory does not exist. Skipping.');
    return;
  }

  const images = await processDirectory(config.directories.sketchesSource);

  for (const imagePath of images) {
    // Maintain the directory structure in the output
    const relativePath = path.relative(config.directories.sketchesSource, imagePath);
    const outputPath = path.join(config.directories.optimized, 'sketches', path.dirname(relativePath));
    ...
    try {
      // Sketches are always square and cropped to center
      await sharp(imagePath)
        .resize({
          width: config.sketches.square.width,
          height: config.sketches.square.height,
          fit: 'cover', // Crop to fill the square
          position: 'center',
        })
        .webp({ quality: config.sketches.square.quality })
        .toFile(outputFilename);
        
      console.log(`  Optimized square sketch: ${relativePath} -> ${path.basename(outputFilename)}${replacementMsg}`);
      ...
      if (!imagePath.includes('thumb')) {
        ...
        await sharp(imagePath)
          .resize({
            width: config.sketches.thumbnail.width,
            height: config.sketches.thumbnail.width,
            fit: 'cover',
            position: 'center',
          })
          .blur(2)
          .webp({ quality: config.sketches.thumbnail.quality })
